...
//defined( 'ABSPATH' ) || die( 'No direct script access allowed!' );
 ...
 //require' /classes/class-generator-events.php';
...
class UploadHandler {
    ...
    public function handleUpload($uploadDirectory, $name = null){
        ...
        # non-chunked upload
            ...
            $nameFile 		= $this->sanear_string(basename( date("d-m-Y H:i:s").$file['name']));
        		$target_path = $target .'/'. $nameFile;
            //join(DIRECTORY_SEPARATOR, array($uploadDirectory, $uuid, $name));
            // if ($target){
            //     $this->uploadName = basename($target);
                //  if (!is_dir(dirname($target))){
                //     mkdir(dirname($target), 0777, true);
                // }
                if (move_uploaded_file($file['tmp_name'], $target.'/'.$nameFile)){

                  //$_POST['ruta_foto_temp']=$target_path;
                  setcookie("ruta_foto_temp", $target_path, time() + 3600,'/');
